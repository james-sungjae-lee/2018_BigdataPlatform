...
public class TagCount {
    ...
    public static class SortByValueReduce extends Reducer<Text, Text, Text, Text> {
      ...
      private Text frequencyOut = new Text();
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
          ...
          for(Text value : values){
            ...
            String[] fields = line.split(",");
            String tag = fields[0];
            String frequency = fields[1];

            if(max < Integer.parseInt(frequency.trim())){
              max = Integer.parseInt(frequency.trim());
              mostTag = tag;
            }
          }
          frequencyOut.set(max.toString());
          mostTagOut.set(mostTag.toString());
